...
export interface RoomCardProps {
  ...
  statusLabel?: string;
  imageUrl: string;
  amenities: { icon: string; rotateClass: string; bgClass: string }[];
  actionLabel: string;
  actionLink?: string;
  actionClass?: string;
  imageOpacity?: string;
...
export function RoomCard({ 
  id, title, capacity, location, status, statusLabel, imageUrl, amenities, actionLabel, actionLink, actionClass, imageOpacity = "opacity-80 group-hover:opacity-100" 
}: RoomCardProps) {
  ...
  return (
    <Card interactive className="group">
      <div className="h-40 bg-surface-variant border-b-[3px] border-on-surface relative overflow-hidden">
        ...
        <img 
            alt="Room interior" 
            className={`w-full h-full object-cover grayscale transition-all duration-300 ${status === 'occupied' ? 'mix-blend-multiply opacity-60' : 'group-hover:grayscale-0 group-hover:scale-105'} ${imageOpacity}`} 
            src={imageUrl}
        />
        {getStatusBadge()}
      </div>
      ...
      <CardContent>
        ...
        <div className="flex gap-3 mb-6">
          {amenities.map((am, i) => (
            <div key={i} className={`w-10 h-10 rounded-DEFAULT neo-border flex items-center justify-center transition-transform hover:rotate-0 ${am.bgClass} ${am.rotateClass}`}>
              <Icon name={am.icon} className="doodle-icon text-[24px]" />
            </div>
          ))}
        </div>
        ...
        <div className="mt-auto">
          {actionLink ? (
            <Link to={actionLink} className={`block w-full text-center font-label-bold font-bold text-[14px] py-3 neo-border neo-shadow transition-all rounded-DEFAULT ${actionClass || 'bg-primary text-on-primary neo-shadow-hover neo-shadow-active'}`}>
              {actionLabel}
            </Link>
          ) : (
            <button className={`w-full font-label-bold font-bold text-[14px] py-3 neo-border neo-shadow transition-all rounded-DEFAULT ${actionClass || 'bg-primary text-on-primary neo-shadow-hover neo-shadow-active'}`}>
              {actionLabel}
            </button>
          )}
        </div>
