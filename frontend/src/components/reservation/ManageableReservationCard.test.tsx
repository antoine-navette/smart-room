...
function renderCard(
  overrides: Partial<ComponentProps<typeof ManageableReservationCard>> = {},
) {
...
describe('ManageableReservationCard', () => {
  ...
  it('affiche les informations principales de la reservation', () => {
    ...
    expect(screen.getByRole('link', { name: 'Voir la salle' })).toHaveAttribute(
      'href',
      '/room/10',
    );
  });
  ...
  it('ouvre la modale de modification', () => {
    ...
    expect(
      screen.getByRole('dialog', { name: 'Modifier la reservation' }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        'Tu peux ajuster la salle et le creneau tant que la reservation est encore a venir.',
      ),
    ).toBeInTheDocument();
    expect(
  ...
  it('annule une reservation et remonte le succes', async () => {
    ...
    expect(onFeedback).toHaveBeenLastCalledWith(
      'success',
      'La reservation a bien ete annulee.',
    );
  });
  ...
  it('bloque la sauvegarde si le nouveau debut est dans le passe', async () => {
    ...
    await waitFor(() => {
      expect(onFeedback).toHaveBeenLastCalledWith(
        'error',
        'Le nouveau debut doit rester dans le futur.',
      );
    });
  ...
  it('sauvegarde une modification valide', async () => {
    ...
    expect(onFeedback).toHaveBeenLastCalledWith(
      'success',
      'La reservation a bien ete modifiee.',
    );
  });
});