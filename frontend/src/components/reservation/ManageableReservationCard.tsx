...
export function ManageableReservationCard({
  ...
  const isCurrentlyRunning =
    new Date(reservation.start_time).getTime() <= Date.now() && Date.now() < new Date(reservation.end_time).getTime();

  const flexDirection = 'md:flex-row';
  ...
  const handleSaveEdit = async () => {
    ...
    const selectedRoomId = allowRoomChange ? Number(editRoomId) : (reservation.room_id ?? editableRooms[0]?.id ?? null);
  ...
  return (
    <article className="bg-surface-container-lowest border-[3px] border-on-surface neo-shadow flex flex-col md:flex-row">
      <div className="md:w-1/3 p-6 flex flex-col justify-center items-center border-b-[3px] md:border-b-0 md:border-r-[3px] border-on-surface bg-primary-fixed text-on-primary-fixed">
        <span className="font-headline-xl font-black text-[42px] leading-none">
          {formatTime(reservation.start_time)}
        </span>
        <span className="font-label-bold font-bold uppercase mt-2">{formatDayLabel(reservation.start_time)}</span>
      </div>
      ...
      <div className="p-6 md:w-2/3 flex flex-col gap-4">
        ...
        {isEditing ? (
          <div className="bg-surface border-[3px] border-on-surface p-4 flex flex-col gap-4">
            <h4 className="font-label-bold font-bold uppercase text-on-surface-variant">Modifier la reservation</h4>
                ...
                >
                  {editableRooms.map((availableRoom) => (
                    <option key={availableRoom.id} value={String(availableRoom.id)}>
                      {availableRoom.name} - {buildLocation(availableRoom, floorById, buildingById)}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                disabled={isSavingEdit}
                className="bg-primary text-on-primary font-label-bold font-bold px-4 py-2 border-[3px] border-on-surface neo-shadow-sm neo-active-sm disabled:opacity-60 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={isSavingEdit}
                className="bg-surface text-on-surface font-label-bold font-bold px-4 py-2 border-[3px] border-on-surface neo-shadow-sm neo-active-sm disabled:opacity-60 disabled:cursor-not-allowed"
              >
