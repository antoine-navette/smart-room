  ...
  return (
    <div className="bg-surface-container-lowest border-[3px] border-on-surface p-4 flex flex-col gap-2 relative overflow-hidden group">
      ...
      <div className="flex justify-between items-start">
        ...
        <span className={`${status === 'Pending' ? 'bg-error-container text-on-error-container' : 'bg-secondary-fixed text-on-secondary-fixed'} font-label-bold font-bold px-2 py-1 border-[2px] border-on-surface`}>
          {status}
        </span>
      </div>
      <p className="font-body-md text-on-surface-variant">{location} • {details}</p>
