...
export function BookingCard({ time, meridiem, title, status, capacity, hasVc, isUpcoming }: BookingCardProps) {
  ...
  const statusBgClass = isUpcoming ? "bg-secondary-fixed text-on-secondary-fixed" : "bg-tertiary-fixed text-on-tertiary-fixed";

  return (
    <div className="bg-surface-container-lowest border-[3px] border-on-surface neo-shadow flex flex-col md:flex-row mt-4">
      <div className={`md:w-1/3 p-6 flex flex-col justify-center items-center border-b-[3px] md:border-b-0 md:border-r-[3px] border-on-surface ${timeBgClass}`}>
