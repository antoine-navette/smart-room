...
export const roomUnavailabilityRoutes: FastifyPluginAsyncZodOpenApi<Options> = async (app, { authService, roomUnavailabilityService }) => {
    app.post('/room-unavailabilities', {
        schema: {
        ...
        const result = await roomUnavailabilityService.create(auth.user, body.data.room_id, body.data.from_time, body.data.to_time, body.data.reason);
        if (!result.success) {
            if (result.code === 'FORBIDDEN') return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to create a room unavailability' });
            if (result.code === 'ROOM_NOT_FOUND') return reply.status(404).send({ code: 'ROOM_NOT_FOUND', message: 'Room not found' });
            if (result.code === 'INVALID_DATE_RANGE') return reply.status(400).send({ code: 'INVALID_DATE_RANGE', message: 'from_time must be before to_time' });
            if (result.code === 'ROOM_UNAVAILABILITY_CONFLICT') return reply.status(409).send({ code: 'ROOM_UNAVAILABILITY_CONFLICT', message: 'Room unavailability overlaps with an existing one' });
    ...
    app.get('/room-unavailabilities/:id', {
        schema: {
            tags: ['RoomUnavailabilities'],
        ...
        if (!result.success) return reply.status(404).send({ code: 'ROOM_UNAVAILABILITY_NOT_FOUND', message: 'Room unavailability not found' });

        return reply.status(200).send(result.roomUnavailability);
    });

    app.get('/rooms/:roomId/unavailabilities', {
        schema: {
            tags: ['RoomUnavailabilities'],
    ...
    app.put('/room-unavailabilities/:id', {
        schema: {
            tags: ['RoomUnavailabilities'],
            params: RoomUnavailabilityIdParamsDto,
        ...
        const result = await roomUnavailabilityService.update(auth.user, params.data.id, body.data.room_id, body.data.from_time, body.data.to_time, body.data.reason);
        if (!result.success) {
            if (result.code === 'FORBIDDEN') return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to update a room unavailability' });
            if (result.code === 'ROOM_NOT_FOUND') return reply.status(404).send({ code: 'ROOM_NOT_FOUND', message: 'Room not found' });
            if (result.code === 'ROOM_UNAVAILABILITY_NOT_FOUND') return reply.status(404).send({ code: 'ROOM_UNAVAILABILITY_NOT_FOUND', message: 'Room unavailability not found' });
            if (result.code === 'INVALID_DATE_RANGE') return reply.status(400).send({ code: 'INVALID_DATE_RANGE', message: 'from_time must be before to_time' });
            if (result.code === 'ROOM_UNAVAILABILITY_CONFLICT') return reply.status(409).send({ code: 'ROOM_UNAVAILABILITY_CONFLICT', message: 'Room unavailability overlaps with an existing one' });
            result satisfies never;
        }
    ...
    app.delete('/room-unavailabilities/:id', {
        schema: {
            tags: ['RoomUnavailabilities'],
            params: RoomUnavailabilityIdParamsDto,
            response: {
        ...
        if (!result.success) {
            if (result.code === 'FORBIDDEN') return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to delete a room unavailability' });
            if (result.code === 'ROOM_UNAVAILABILITY_NOT_FOUND') return reply.status(404).send({ code: 'ROOM_UNAVAILABILITY_NOT_FOUND', message: 'Room unavailability not found' });
            result satisfies never;
        }
