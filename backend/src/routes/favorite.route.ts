...
import {
    ...
    FavoriteUserIdParamsDto,
    FavoriteUserRoomParamsDto,
    ...
    app.post(
        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                ...
                if (result.code === 'FAVORITE_ALREADY_EXISTS')
                    return reply
                        .status(409)
                        .send({ code: 'FAVORITE_ALREADY_EXISTS', message: 'Favorite already exists' });
        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN')
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You can only delete your own favorites' });
                if (result.code === 'FAVORITE_NOT_FOUND')
                    return reply.status(404).send({ code: 'FAVORITE_NOT_FOUND', message: 'Favorite not found' });
                result satisfies never;
            }
