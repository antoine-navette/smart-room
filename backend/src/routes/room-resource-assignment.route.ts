...
import { RoomResourceAssignmentDto, SetRoomResourceQuantityBodyDto, RoomResourceAssignmentParamsDto } from '../schemas/room-resource-assignment.schema.js';
import {
    ForbiddenErrorDto,
...
export const roomResourceAssignmentRoutes: FastifyPluginAsyncZodOpenApi<Options> = async (app, { authService, roomResourceAssignmentService }) => {
    app.get('/rooms/:roomId/resources', {
        schema: {
            tags: ['Rooms'],
    ...
    app.put('/rooms/:roomId/resources/:resourceId', {
        schema: {
            tags: ['Rooms'],
        ...
        const result = await roomResourceAssignmentService.setQuantity(auth.user, params.data.roomId, params.data.resourceId, body.data.quantity);
        if (!result.success) {
            if (result.code === 'FORBIDDEN') {
                return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to update room resources' });
            }
    ...
    app.delete('/rooms/:roomId/resources/:resourceId', {
        schema: {
            tags: ['Rooms'],
            params: RoomResourceAssignmentParamsDto,
            response: {
                ...
                401: UnauthorizedErrorDto,
                403: ForbiddenErrorDto,
                404: z.union([RoomNotFoundErrorDto, RoomResourceNotFoundErrorDto, RoomResourceAssignmentNotFoundErrorDto]),
                500: InternalServerErrorDto,
            },
        },
    }, async (request, reply) => {
        ...
        const result = await roomResourceAssignmentService.delete(auth.user, params.data.roomId, params.data.resourceId);
        if (!result.success) {
            if (result.code === 'FORBIDDEN') {
                return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to delete room resources' });
            }
            if (result.code === 'ROOM_NOT_FOUND') {
                return reply.status(404).send({ code: 'ROOM_NOT_FOUND', message: 'Room not found' });
            }
            ...
            if (result.code === 'ROOM_RESOURCE_ASSIGNMENT_NOT_FOUND') {
                return reply.status(404).send({ code: 'ROOM_RESOURCE_ASSIGNMENT_NOT_FOUND', message: 'Room resource assignment not found' });
            }
            result satisfies never;
        }
