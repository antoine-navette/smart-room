...
import { RoomResourceDto, CreateRoomResourceBodyDto, UpdateRoomResourceBodyDto, RoomResourceIdParamsDto } from '../schemas/room-resource.schema.js';
import {
    ForbiddenErrorDto,
    InvalidBodyErrorDto,
...
export const roomResourceRoutes: FastifyPluginAsyncZodOpenApi<Options> = async (app, { authService, roomResourceService }) => {
    app.post('/room-resources', {
        schema: {
            tags: ['RoomResources'],
        ...
        if (!result.success) {
            if (result.code === 'FORBIDDEN') {
                return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to create a room resource' });
            }
            if (result.code === 'ROOM_RESOURCE_NAME_EXISTS') {
                return reply.status(409).send({ code: 'ROOM_RESOURCE_NAME_EXISTS', message: 'Room resource name already exists' });
            }
    ...
    app.put('/room-resources/:id', {
        ...
        if (!result.success) {
            if (result.code === 'FORBIDDEN') {
                return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to edit a room resource' });
            }
            if (result.code === 'ROOM_RESOURCE_NAME_EXISTS') {
                return reply.status(409).send({ code: 'ROOM_RESOURCE_NAME_EXISTS', message: 'Room resource name already exists' });
            }
    ...
    app.delete('/room-resources/:id', {
        ...
        if (!result.success) {
            if (result.code === 'FORBIDDEN') {
                return reply.status(403).send({ code: 'FORBIDDEN', message: 'You must be an admin to delete a room resource' });
            }
            if (result.code === 'ROOM_RESOURCE_NOT_FOUND') {
                return reply.status(404).send({ code: 'ROOM_RESOURCE_NOT_FOUND', message: 'Room resource not found' });
            }
