        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN') {
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to create a building' });
                }
                if (result.code === 'BUILDING_NAME_EXISTS') {
                    return reply
                        .status(409)
                        .send({ code: 'BUILDING_NAME_EXISTS', message: 'Building name already exists' });
                }
        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN') {
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to edit a building' });
                }
                if (result.code === 'BUILDING_NAME_EXISTS') {
                    return reply
                        .status(409)
        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN') {
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to delete a building' });
                }
                if (result.code === 'BUILDING_NOT_FOUND') {
                    return reply.status(404).send({ code: 'BUILDING_NOT_FOUND', message: 'Building not found' });
                }
                if (result.code === 'BUILDING_HAS_FLOORS') {
                    return reply
                        .status(409)
                        .send({ code: 'BUILDING_HAS_FLOORS', message: 'Building still has floors' });
                }
                result satisfies never;
            }
