...
export const incidentRoutes: FastifyPluginAsyncZodOpenApi<Options> = async (app, { authService, incidentService }) => {
    ...
    app.get(
        ...
        async (request, reply) => {
            ...
            if (!result.success)
                return reply.status(404).send({ code: 'INCIDENT_NOT_FOUND', message: 'Incident not found' });
    ...
    app.put(
        ...
        async (request, reply) => {
            ...
            const result = await incidentService.update(
                ...
                body.data.title,
                body.data.description,
                body.data.status as any,
            );
            if (!result.success) {
                if (result.code === 'FORBIDDEN')
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to update an incident' });
                if (result.code === 'INCIDENT_NOT_FOUND')
                    return reply.status(404).send({ code: 'INCIDENT_NOT_FOUND', message: 'Incident not found' });
                if (result.code === 'INCIDENT_ALREADY_RESOLVED')
                    return reply
                        .status(409)
        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN')
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to delete an incident' });
                if (result.code === 'INCIDENT_NOT_FOUND')
                    return reply.status(404).send({ code: 'INCIDENT_NOT_FOUND', message: 'Incident not found' });
                result satisfies never;
            }
