        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN') {
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to create a room' });
                }
        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN') {
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to edit a room' });
                }
                if (result.code === 'ROOM_NAME_EXISTS') {
                    return reply.status(409).send({ code: 'ROOM_NAME_EXISTS', message: 'Room name already exists' });
                }
        ...
        async (request, reply) => {
            ...
            if (!result.success) {
                if (result.code === 'FORBIDDEN') {
                    return reply
                        .status(403)
                        .send({ code: 'FORBIDDEN', message: 'You must be an admin to delete a room' });
                }
                if (result.code === 'ROOM_NOT_FOUND') {
                    return reply.status(404).send({ code: 'ROOM_NOT_FOUND', message: 'Room not found' });
                }
